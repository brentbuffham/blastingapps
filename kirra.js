...
// Last Modified: "20250603.2052AWST"
const buildVersion = "20250603.2052AWST"; //Backwards Compatible Date Format
//-----------------------------------------
...
function handleMouseMove(event) {
	...
	if (selectionMode && selectedMultipleHoles.length > 0) {
		isUpdatingSelectionFromMove = true;
	}
...
function drawHoleMainShape(point, x, y, selectedHole) {
	...
	if (selectedHole != null && selectedHole === point) {
		...
		highlightColor1 = "rgba(255, 0, 150, 0.2)";
		highlightColor2 = "rgba(255, 0, 150, .8)";
		// Check if the current point is the first one in the selectedMultipleHoles array.
		// This ensures the text is effectively "printed" only once, associated with the first selected hole.
		// The `find` condition ensures selectedMultipleHoles is not empty if this block is entered.
		if (point === selectedMultipleHoles[0]) {
			highlightText = "Editing Selected Holes: {" + selectedMultipleHoles.map((h) => h.holeID).join(",") + "}";
		} else {
			// For other selected holes, highlight the hole shape but don't repeat the text.
			highlightText = "";
		}
	}
	...
	Swal.fire({
		...
		<path d="M65.76,119.94c0,0-0.8-2.02-2.61-2.63c-1.81-0.61-4.99-0.91-6.88-0.92s-3.11,0.63-4.16,0c-1.05-0.63-1.66-1.39-1.66-1.39" />
			...
			<br>
				...
				<br><label class="labelWhite18">Floating Toolbar added</label>
				<br><label class="labelWhite18">Selection by Pointer and Polygon</label>
				<br><label class="labelWhite18">Location crosshair</label>
